...
from generators.content_generator import generate_backlink_content
...
def _handle_generate_titles(state: dict):
    ...
    if not state["selected_backlink_id"].isdigit():
        state["error"] = "Please choose a backlink site first."
        return

    state["selected_backlink"] = get_backlink(int(state["selected_backlink_id"]))
    if not state["selected_backlink"]:
        state["error"] = "The selected backlink site could not be found."
        return
    ...
    if not state["brand"]:
        state["error"] = "Please select a brand for the backlink article."
        return

    if not state["brand_website_url"]:
        state["error"] = "The selected brand needs a website saved in Brands before generating backlink content."
        return
    ...
    try:
        ...
        state["error"] = generation_error_message(
            "An error occurred while generating backlink article titles. Check logs/app.log for details.",
            exc,
        )
...
def _handle_generate_content(state: dict):
    ...
    try:
        ...
        state["error"] = generation_error_message(
            "An error occurred while generating backlink article content. Check logs/app.log for details.",
            exc,
        )
